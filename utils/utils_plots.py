...
def plot_time_series_by_category(df, category_col, target_col, time_col, agg_func='sum', window=None, legend=True):
    """
    ...
    """
    if time_col not in df.columns or category_col not in df.columns or target_col not in df.columns:
    ...
    grouped = df.groupby([time_col, category_col])[target_col].agg(agg_func).reset_index()
